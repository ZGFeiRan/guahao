...
import com.guahao.hancw.common.FieldHelper;
import com.guahao.hancw.common.PsiClassUtil;
import com.guahao.hancw.common.Try;
import com.guahao.hancw.config.Config;
import com.guahao.hancw.config.Constant;
import com.guahao.hancw.entity.ClassEntity;
import com.guahao.hancw.entity.ConvertLibrary;
import com.guahao.hancw.entity.FieldEntity;
import com.guahao.hancw.entity.IterableFieldEntity;
...
import static com.guahao.hancw.common.StringUtils.captureName;
...
//        generateGetterAndSetter(factory, cls, classEntity);
    ...
    private void generateConstructMethod(PsiElementFactory factory, PsiClass cls,ClassEntity clsEntity) {
        String classNames = cls.getName();
        String methodLine = "public ".concat(classNames).concat("( ")
                .concat("JSONObject obj").concat(") throws JSONException {   ".concat("\n"));
        List<? extends FieldEntity> fieldEntities = clsEntity.getFields();

        //生成解析行
        for (FieldEntity fieldEntity : fieldEntities) {
            String fieldTypeLine = getTypeLine(fieldEntity);
           methodLine=methodLine.concat(fieldEntity.getGenerateFieldName()).concat(" = ")
                   .concat(fieldTypeLine).concat("\n");
        }

        String finalMethod = methodLine.concat("}");
        ...
        Try.run(new Try.TryListener() {
            ...
            @Override
            public void runAgain() {
//                cls.addBefore(factory.createCommentFromText("// FIXME generate failure  method  constructor " + captureName(finalFieldName), cls), cls.getChildren()[0]);
    ...
    private String getTypeLine(FieldEntity fieldEntity) {
        ...
        if (fieldEntity.getFullNameType().equals("long")||fieldEntity.getFullNameType().equals("Long")){
            ...
            //如果是数组JSONArray,生成classFieldName =new ArrayList();植入以下模板：
            /**
             if(array!=null&&array.length()>0){
             for(int i=0;i<array.length();i++){
             JSONObject itemObj =array.getJSONObject(i);
             resultList.add(new InnerClassName(itemObj)
             }
             }
             */
            //默认数组处理
            if (fieldEntity instanceof IterableFieldEntity) {
                typeLine = "new ArrayList();" +"\n"
                        +"JSONArray array =obj.optJSONArray(\""+fieldEntity.getFieldName()+"\");"+"\n";
                if (!isBaseType((IterableFieldEntity) fieldEntity)) {
                    //不是基本类型,new className(JSONObject)
                    typeLine = typeLine + "if(array!=null&&array.length()>0) {" + "\n"
                            + "for(int i=0;i<array.length();i++){" + "\n"
                            + "JSONObject itemObj=array.getJSONObject(i);" + "\n"
                            + fieldEntity.getFieldName() + ".add(new "
                            + ((IterableFieldEntity) fieldEntity).getParameterClassName()
                            + "(itemObj));" + "\n"
                            + "}" + "\n"
                            + "}";
                } else {
                    ...
                    typeLine = typeLine + "if(array!=null&&array.length()>0) {" + "\n"
                            + "for(int i=0;i<array.length();i++){" + "\n"
                            + fieldEntity.getFieldName() + ".add(("
                            + ((IterableFieldEntity) fieldEntity).getParameterClassName()+")array.get(i));" + "\n"
                            + "}" + "\n"
                            + "}";
                }
            } else {
    ...
    private boolean isBaseType(IterableFieldEntity fieldEntity) {
        String type =fieldEntity.getRealType();
        if (type.equals("List<Integer>")
                || type.equals("List<String>")
                || type.equals("List<Long>")
                || type.equals("List<Boolean>")) {
            return true;
        }
        return false;
    }
    ...
    protected void createGetAndSetMethod(PsiElementFactory factory, PsiClass cls, FieldEntity field) {
        if (field.isGenerate()) {
            String fieldName = field.getGenerateFieldName();//属性名称
    ...
    protected void generateClass(PsiElementFactory factory, ClassEntity classEntity, PsiClass parentClass, IProcessor visitor) {
        ...
        if (classEntity.isGenerate()) {
            ...
            if (generateClass != null) {
               //解析规则
                for (ClassEntity innerClass : classEntity.getInnerClasss()) {//生成所需内部类
                    generateClass(factory, innerClass, generateClass, visitor);
                }
                if (!Config.getInstant().isSplitGenerate()) {
                    generateClass = (PsiClass) parentClass.add(generateClass);
                }
                for (FieldEntity fieldEntity : classEntity.getFields()) {//生成属性集合
...
//                generateGetterAndSetter(factory, generateClass, classEntity);
                ...
                generateConstructMethod(factory,generateClass,classEntity);
//     generateConstructMethodInnerClass(factory,generateClass,classEntity);
            }
        }
    ...
    private String generateFieldText(ClassEntity classEntity, FieldEntity fieldEntity, String fixme) {
        ...
        String filedName = fieldEntity.getGenerateFieldName();//属性名称
